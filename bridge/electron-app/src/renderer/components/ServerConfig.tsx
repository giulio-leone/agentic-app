      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <div className="flex gap-1">
            <input
              type="text"
              className="flex-1 bg-surface border border-gray-600 rounded-l px-2 py-1.5 text-sm text-gray-200 focus:border-accent focus:outline-none"
              value={config.cwd}
              onChange={(e) => onUpdate({ cwd: e.target.value })}
              placeholder="~/"
            />
            <button
              className="px-2 py-1.5 bg-accent hover:bg-accent-light text-white text-xs rounded-r transition-colors"
