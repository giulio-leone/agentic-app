...
import { Wifi, QrCode, Settings, CheckCircle, XCircle, Server, MessageSquare as MessageSquareIcon } from 'lucide-react-native';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import * as Haptics from 'expo-haptics';
import { useDesignSystem } from '../utils/designSystem';
import { FontSize, Spacing, Radius } from '../utils/theme';
import { sharedStyles } from '../utils/sharedStyles';
import CliSessionsTab from '../components/copilot/CliSessionsTab';
import {
...
export default function CopilotBridgeScreen({ route }: Props) {
  ...
  const fetchBridgeInfo = useCallback(async () => {
    ...
    try {
      ...
      // Info card won't show — don't retry, reconnect will trigger again
    }
  }, [bridge]);

  useEffect(() => {
    if (isAuthenticated && !bridgeInfo) fetchBridgeInfo();
    if (!isAuthenticated) setBridgeInfo(null);
  }, [isAuthenticated, fetchBridgeInfo, bridgeInfo]);
    ...
    >
      ...
      >
        ...
        <XStack
          ...
          paddingVertical={Spacing.sm}
        >
          <XStack backgroundColor={colors.systemGray5} borderRadius={Radius.sm} padding={2} flex={1}>
            {([
              { key: 'settings' as const, label: 'Connection', Icon: Settings },
              { key: 'sessions' as const, label: 'CLI Sessions', Icon: MessageSquareIcon },
            ]).map(({ key, label, Icon }) => (
              <TouchableOpacity
                key={key}
                style={[
              ...
              >
                <Icon size={12} color={activeTab === key ? colors.text : colors.textTertiary} />
                <Text
                  fontSize={FontSize.caption}
                  fontWeight={activeTab === key ? '600' : '500'}
                  color={activeTab === key ? '$color' : '$textTertiary'}
                >
                  {label}
                </Text>
              </TouchableOpacity>
        ...
        {activeTab === 'sessions' && (
          <CliSessionsTab
            ...
            onResumeSession={(cliSessionId, bridgeSessionId, model) => {
              Alert.alert(
                'Session Resumed',
                `CLI Session: ${cliSessionId.slice(0, 8)}…\nBridge Session: ${bridgeSessionId}\nModel: ${model}`,
              );
            }}
          />
        )}
